#!/usr/bin/env python3
"""Measure exiD baseline/+I latency for MTP-GO adapter."""
...
CASES = [("exiD-baseline", "exiD0-5/best.ckpt"), ("exiD-+I", "exiD2-5/best.ckpt")]
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description=__doc__)
    p.add_argument("--data-root", type=Path)
    p.add_argument("--ckpt-root", type=Path, default=Path("ckpts/mtp_go"))
    ...
    p.add_argument("--log-dir", type=Path, default=Path("logs/latency/mtp_go"))
...
def main() -> int:
    ...
    if device == "auto":
        ...
        device = probe.stdout.strip() if probe.returncode == 0 and probe.stdout.strip() else "cpu"
    failures = 0
