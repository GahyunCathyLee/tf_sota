"""Helpers for importing the official QCNet checkout without vendoring it."""
...
def resolve_upstream_dir(path: str | Path | None = None) -> Path:
    upstream = Path(path) if path is not None else EXPERIMENT_ROOT / "external" / "qcnet"
    upstream = upstream.expanduser()
    if not upstream.is_absolute():
        upstream = (EXPERIMENT_ROOT / upstream).resolve()
    if not upstream.exists():
        raise FileNotFoundError(
            f"QCNet upstream checkout not found: {upstream}\n"
            "Clone it with: git clone https://github.com/ZikangZhou/QCNet.git external/qcnet"
        )
    return upstream
