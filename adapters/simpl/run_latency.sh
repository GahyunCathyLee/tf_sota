...
# Optional data override: DATA_ROOT=/path/to/sota_experiments_data ./adapters/simpl/run_latency.sh
# Optional checkpoint overrides:
#   CKPT_ROOT=/path/to/simpl_ckpts ./adapters/simpl/run_latency.sh
#   EXID_BASE_CKPT=/path/to/exiD0-5.pt EXID_I_CKPT=/path/to/exiD2-5.pt ./adapters/simpl/run_latency.sh
...
for row in "${cases[@]}"; do
  ...
  if [[ -n "$ckpt_key" ]]; then
    ...
    ckpt="${CKPT_ROOT}/${ckpt#ckpts/simpl/}"
  fi
